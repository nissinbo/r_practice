...
sample_patient <- data.frame(id = 1:n) %>% 
  ...
  mutate(end_date = as.Date("2020/12/31")) %>% 
  mutate(age = sample(41:51, 10))
...
setwd("./R_practice/data/")

fwrite(sample_patient, "patient.csv")
fwrite(sample_drug, "drug.csv")
fwrite(sample_disease, "disease.csv")
